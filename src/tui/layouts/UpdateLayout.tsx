...
function buildFocusItems(width: number, height: number, mainWidth: number, sideWidth: number, stacked: boolean): FocusItem[] {
  if (stacked) {
    return [
      { id: "packages", row: 0, column: 0, redirectTo: "input", bounds: { x: 1, y: 2, width, height: Math.max(8, height - 11) } },
      { id: "input", row: 1, column: 0, parentIds: ["packages"], bounds: { x: 3, y: Math.max(4, height - 12), width: width - 4, height: 3 } },
      { id: "summary", row: 2, column: 0, bounds: { x: 1, y: Math.max(3, height - 9), width, height: 8 } },
    ];
  }
  return [
    { id: "packages", row: 0, column: 0, redirectTo: "input", bounds: { x: 1, y: 2, width: mainWidth, height: height - 2 } },
    { id: "input", row: 1, column: 0, parentIds: ["packages"], bounds: { x: 3, y: Math.max(4, height - 4), width: mainWidth - 4, height: 3 } },
